...
import { GetApplicationTrademarkListService } from "../../services/globalServices";
...
const AppList = () => {
  ...
  const [status, setStatus] = useState(EnumForStatus.All);
  const columns = [
    {
      name: "Denomination",
    ...
    {
      name: "GPA Number",
      selector: (row) => row?.gpaNumber ? row?.gpaNumber : "N/A",
      sortable: true,
    },
    // {
        ...
        <div className="d-flex position-relative">
          <OverlayTrigger placement="top" overlay={<Tooltip>Detail</Tooltip>}>
            <NavLink
              ...
              to={`${pathData.applicationDetail}${row?.uniqueReference}`}
              onClick={() => dispatch(currentPagehandler(currentPage)) & dispatch(searchHandler(search)) & dispatch(searchHandlerFlag("App management"))}
            >
              <FaEye />
            </NavLink>
          </OverlayTrigger>
  ...
  // })
  const statusOptions = [
    { value: EnumForStatus.All, label: "ALL" },
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      delayDebounce = setTimeout(() => {
        GetApplicationTrademarkListService(
          setData,
          setLoading,
          setTotalPages,
          currentPage,
  ...
  useEffect(() => {
    ...
    if (searchFlag === "User management" || searchFlag === "Trademark management") {
  ...
  return (
    <div>
      <CustomDatatable
        ...
        data={data || []}
        columns={columns}
        headtitle="APPLICATION MANAGEMENT"
        search={search}
        // statusfilter={status}
